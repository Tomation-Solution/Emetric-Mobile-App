...
export default function IconCard(props) {
  return (
    ...
    <View style={[tw`${props.bg} px-2 py-5 mx-1 my-2 h-24 rounded-xl ${props.width?'w-5/12':'w-11/12'} flex-row`,]}>
            {props.icon}
            <View>
              ...
              <View style={tw`flex-row justify-between pt-2`}>
                ...
                <View style={tw`w-7/12  my-auto ${props.amount<100?'ml-3':'-ml-3'} bg-blue-200 h-1.5 rounded-full`}>
                {/* <View style={[tw` my-auto bg-blue-800 h-1.5 rounded-full`, {width:props.amount+'%'}]}> */}
                  <View style={[tw` my-auto bg-blue-800 h-1.5 rounded-full`,{width: parseInt(props.amount)}]}>
